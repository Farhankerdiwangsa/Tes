...
export default function Latest() {
  ...
  const [bannerAnime, setBannerAnime] = useState<Anime | null>(null);
  const [currentPage, setCurrentPage] = useState(1);
  const [loadingMore, setLoadingMore] = useState(false);

  const API_URL = process.env.NEXT_PUBLIC_API_BASE_URL + '/latest?page=';
  ...
  const loadLatestAnime = async (page: number, isFirstPage = false) => {
    if (isFirstPage) {
      setLoading(true);
    } else {
      setLoadingMore(true);
  ...
  const handleRetry = () => {
    setError(false);
    loadLatestAnime(1, true);
  };

  const handleLoadMore = () => {
    loadLatestAnime(currentPage, false);
  };
    ...
    <div className="min-h-screen bg-panel">
      <section id="banner-section" className="bg-panel shadow-lg overflow-hidden relative w-full pt-6 px-4 md:px-8">
        <div id="banner-container" className="relative">
          {bannerAnime && createBannerSlide(
            bannerAnime.title,
            bannerAnime.imageUrl,
            bannerAnime.detailLink,
            "Terbaru",
            "Temukan anime terbaru yang baru rilis!",
            bannerAnime.imageUrl
          )}
        </div>
      </section>
          ...
          <>
            ...
            <div className="text-center mt-8 mb-4">
              <button 
                ...
                className="bg-mustard font-bangers text-2xl px-8 py-3 comic-border rounded-md comic-shadow text-textblack tracking-wider transition-transform duration-200 hover:scale-105 active:scale-95 flex items-center justify-center mx-auto disabled:opacity-70 disabled:cursor-default"
              >
                <span>{loadingMore ? 'Memuat...' : 'Muat Lebih Banyak'}</span>
                {loadingMore && (
                  <i className="fa-solid fa-rotate fa-spin ml-2"></i>
                )}
              </button>
            </div>
          </>
